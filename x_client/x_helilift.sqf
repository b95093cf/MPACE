	...
	if ((driver _vehicle) == player) then {
		...
		if (!(_vehicle getVariable "D_Vehicle_Attached") && (_pos select 2 > 2.5) && (_pos select 2 < 11)) then {
			...
			if (!(isNull _nearest) && _nearest != (_vehicle getVariable "D_Attached_Vec")) then {
				_depl = _nearest getVariable "D_MHQ_Deployed";
				...
				if (!_depl) then {
					...
					if ((_px <= _nx + 10 && _px >= _nx - 10) && (_py <= _ny + 10 && _py >= _ny - 10)) then {
						if (!_menu_lift_shown) then {
							_id = _vehicle addAction ["Lift vehicle" call XBlueText, "x_client\x_heli_action.sqf",-1,100000];
			...
			if (isNull _nearest) then {
				...
				if (_vehicle getVariable "D_Vehicle_Attached") then {
					_release_id = _vehicle addAction ["Release vehicle" call XRedText, "x_client\x_heli_release.sqf",-1,100000];
					...
					switch (_nearest) do {
						case MRR1: {
							...
							#ifndef __TT__
							player kbTell [d_kb_logic1,d_kb_topic_side,"Dmr1_in_air",true];
							#else
							player kbTell [d_hq_logic_en1,"HQ_W","Dmr1_in_air",true];
							#endif
						...
						case MRR2: {
							...
							#ifndef __TT__
							player kbTell [d_kb_logic1,d_kb_topic_side,"Dmr2_in_air",true];
							#else
							player kbTell [d_hq_logic_en1,"HQ_W","Dmr2_in_air",true];
							#endif
							["mr2_l_c", _vehicle] call XNetCallEvent;
						};
#ifdef __TT__
						case MRRR1: {
							["mrr1_in_air",true] call XNetSetJIP;
							player kbTell [d_hq_logic_ru1,"HQ_E","Dmr1_in_air",true];
							["mrr1_l_c", _vehicle] call XNetCallEvent;
						};
						case MRRR2: {
							["mrr2_in_air",true] call XNetSetJIP;
							player kbTell [d_hq_logic_ru1,"HQ_E","Dmr2_in_air",true];
							["mrr2_l_c", _vehicle] call XNetCallEvent;
						};
#endif
					};
					...
					switch (_nearest) do {
						case MRR1: {
							...
							#ifndef __TT__
							player kbTell [d_kb_logic1,d_kb_topic_side,"Dmr1_available",true];
							#else
							player kbTell [d_hq_logic_en1,"HQ_W","Dmr1_available",true];
							#endif
						};
						case MRR2: {
							...
							#ifndef __TT__
							player kbTell [d_kb_logic1,d_kb_topic_side,"Dmr2_available",true];
							#else
							player kbTell [d_hq_logic_en1,"HQ_W","Dmr2_available",true];
							#endif
						};
#ifdef __TT__
						case MRRR1: {
							["mrr1_in_air",false] call XNetSetJIP;
							["mrr1_l_c", objNull] call XNetCallEvent;
							player kbTell [d_hq_logic_ru1,"HQ_E","Dmr1_available",true];
						};
						case MRRR2: {
							["mrr2_in_air",false] call XNetSetJIP;
							["mrr2_l_c", objNull] call XNetCallEvent;
							player kbTell [d_hq_logic_ru1,"HQ_E","Dmr2_available",true];
						};
#endif
					};
