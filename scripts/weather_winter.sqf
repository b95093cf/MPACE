...
*/
private ["_pos", "_snow", "_fog", "_enabled", "_snowon"];
...
while {true} do {
	if (!_enabled) then {
		if (__XJIPGetVar(d_winterw) > 0) then {
			_oldval = __XJIPGetVar(d_winterw);
			_enabled = true;
			setWind [2.342, 3.108, true];
			_pphandle ppEffectEnable true;
			_pphandle ppEffectCommit 3;
			if (__XJIPGetVar(d_winterw) == 1) then {
		...
		if (__XJIPGetVar(d_winterw) == 0) then {
			...
			if (__XJIPGetVar(d_winterw) == 1 && !_snowon) then {
				_snow setDropInterval 0.01;
				_fog setDropInterval 0.01;
				_snowon = true;
			} else {
				if (__XJIPGetVar(d_winterw) == 2 && _snowon) then {
					_snowon = false;
					_fog setDropInterval 0;
					_snow setDropInterval 0;
				};
