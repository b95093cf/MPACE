...
if (count _replace > 0) then {
	for "_i" from 0 to (count _objs) - 1 do {
		_ob = _objs select _i;
		if (_ob select 0 == (_replace select 0)) exitWith {_ob set [0, _replace select 1]};
	};
};
...
for "_i" from 0 to ((count _objs) - 1) do {
	if ((random 1) > _rdm) then {
		...
		if ((getNumber(configFile >> "CfgVehicles" >> _type >> "ARTY_IsArtyVehicle")) == 1) then {
			switch (_type) do {
				case "MLRS": {
					_newObj removeMagazine "12Rnd_MLRS";
					_newObj addMagazine "ARTY_12Rnd_227mmHE_M270";
				};
				case "GRAD_RU": {
					_newObj removeMagazine "40Rnd_GRAD";
					_newObj addMagazine "ARTY_40Rnd_120mmHE_BM21";
				};
				case "MLRS_DES_EP1": {
					_newObj removeMagazine "12Rnd_MLRS";
					_newObj addMagazine "ARTY_12Rnd_227mmHE_M270";
			...
			["d_bi_a_v", _newObj] call XNetCallEvent;
		};
#endif
