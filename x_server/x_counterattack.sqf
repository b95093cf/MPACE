...
_vecs_counter_attack = 5 call XfRandomFloor;
...
d_kb_logic1 kbTell [d_kb_logic2,d_kb_topic_side,"CounterattackStarts",true];
...
for "_xx" from 0 to (count _type_list_attack - 1) do {
	...
	if (_nums > 0) then {
		for "_i" from 1 to _nums do {
			[_typeidx select 0, _start_array, _current_target_pos, _typeidx select 1, "attack",d_enemy_side,0,-1.111] spawn x_makegroup;
			sleep 5.123
		};
	};
};
...
sleep 301.122;
d_current_trigger = [_current_target_pos, [350, 350, 0, false], [d_enemy_side, "PRESENT", false], ["(""Tank"" countType thislist  < 2) && (""Man"" countType thislist < 6)", "d_counterattack = false;deleteVehicle d_current_trigger", ""]] call XfCreateTrigger;
